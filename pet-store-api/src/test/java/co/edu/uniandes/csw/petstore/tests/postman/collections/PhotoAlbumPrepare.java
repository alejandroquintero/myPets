...
import co.edu.uniandes.csw.petstore.dtos.detail.PhotoAlbumDetailDTO;
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,PhotoAlbumDetailDTO photoAlbum,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(photoAlbum);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(PhotoAlbumDetailDTO photoAlbum) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:photoAlbum.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(photoAlbum, null).toString().concat("\",\""))); 
           }
