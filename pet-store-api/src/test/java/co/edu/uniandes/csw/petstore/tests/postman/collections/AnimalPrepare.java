 ...
 * @author Asistente
 */
public class AnimalPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionAnimal.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,AnimalDetailDTO animals,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(animals);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(AnimalDetailDTO animals) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:animals.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(animals, null).toString().concat("\",\""))); 
           }
