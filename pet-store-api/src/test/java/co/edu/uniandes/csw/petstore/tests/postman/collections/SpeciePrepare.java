 ...
 */
package co.edu.uniandes.csw.petstore.tests.postman.collections;
 ...
 * @author Asistente
 */
public class SpeciePrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionSpecie.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,SpecieDetailDTO species,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(species);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(SpecieDetailDTO species) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:species.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(species, null).toString().concat("\",\""))); 
           }
