 ...
 * @author Asistente
 */
public class BreedPrepare  {
    
   private static final String PATH = System.getProperty("user.dir").concat("\\collections\\postman_collectionBreed.json");
...
public static void setCollectionBody(JsonElement jsonElement,Integer index,BreedDetailDTO breeds,Gson gson) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{

String val=getFields(breeds);
...
jsonElement.getAsJsonObject().get("item")
    ...
    public static String getFields(BreedDetailDTO breeds) throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
        String open="{\"";
        String content="";
        String close="}";
  for(Method m:breeds.getClass().getMethods()){
           if(m.getName().contains("getName") & !"getClass".equals(m.getName())) {
         content=content.concat(m.getName().substring(3).toLowerCase().concat("\":\"").concat(m.invoke(breeds, null).toString().concat("\",\""))); 
           }
